...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attn = BFPLinear(config.n_embd, 3 * config.n_embd, bias=config.bias)
        # output projection
        self.c_proj = BFPLinear(config.n_embd, config.n_embd, bias=config.bias)
        # regularization
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            att = torch_matmul_bfp(q, k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
            att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
            att = F.softmax(att, dim=-1)
            att = self.attn_dropout(att)
            y = torch_matmul_bfp(att, v) # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = BFPLinear(config.n_embd, 4 * config.n_embd, bias=config.bias)
        self.c_proj  = BFPLinear(4 * config.n_embd, config.n_embd, bias=config.bias)
